// src/components/credentials/HostModal.tsx
...
interface HostModalProps {
    ...
    mode: "create" | "edit"
    host?: HostInfo | null
    onSubmit: (data: CredentialsCreateRequest | CredentialsUpdateRequest) => void
    isSubmitting: boolean
}

export function HostModal({ isOpen, onClose, mode, host, onSubmit, isSubmitting }: HostModalProps) {
    const [esxiHost, setEsxiHost] = useState("")
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!esxiHost.trim() || !username.trim() || !password.trim()) {
        ...
        if (mode === "edit" && host) {
            ...
            const updateData: CredentialsUpdateRequest = {
                esxi_host: esxiHost.trim(),
                username: username.trim(),
                password: password,
                old_username: host.username,
                old_password: oldPassword,
            }
            onSubmit(updateData)
        } else {
            const createData: CredentialsCreateRequest = {
                esxi_host: esxiHost.trim(),
                username: username.trim(),
                password: password,
            }
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8e8e8]">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-[15px] font-semibold text-[#3a3a3a]">
                            {mode === "create" ? "Add ESXi Host" : "Edit Credentials"}
                        </h2>
                    </div>
                    {!isSubmitting && (
                        <button
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            type="text"
                            value={esxiHost}
                            onChange={(e) => setEsxiHost(e.target.value)}
                            placeholder="e.g., esxi-prod-01.lab.local"
