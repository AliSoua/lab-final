// src/components/LabGuide/CreateGuideLab/StepEditor/ContentPreviewStyling.tsx

import { cn } from "@/lib/utils"

interface ContentPreviewStylingProps {
    html: string
    className?: string
}

/**
 * Renders sanitized HTML with platform-native styling.
 * 
 * Strategy:
 * 1. DOMPurify strips ALL inline styles, classes, and dangerous attributes
 * 2. We render the clean structural HTML
 * 3. This CSS block applies our platform design system consistently
 * 
 * This ensures:
 * - Security: no inline styles, event handlers, or scripts
 * - Consistency: all previews look like they belong to the platform
 * - Predictability: instructors focus on content, not styling
 */
export function ContentPreviewStyling({ html, className }: ContentPreviewStylingProps) {
    if (!html?.trim()) {
        return (
            <div className={cn("flex items-center justify-center h-full min-h-[200px] text-[#c4c4c4]", className)}>
                <div className="text-center">
                    <p className="text-sm">Nothing to preview yet</p>
                    <p className="text-[11px] mt-1">Start typing to see rendered output</p>
                </div>
            </div>
        )
    }
