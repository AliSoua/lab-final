# backend/app/services/LabDefinition/task_audit.py
"""
Enhanced task audit service with severity, source, and event codes.
"""

import uuid
import socket
import logging
from datetime import datetime, timezone
from typing import Optional, Dict, Any, Union

from sqlalchemy.orm import Session

from app.utils.db_session import background_session
from app.core.logging import log_task
from app.models.LabDefinition.LabInstanceTask import LabInstanceTask
from app.models.LabDefinition.LabInstanceEventLog import LabInstanceEventLog
from app.models.LabInstance.enums import EventSeverity, EventSource

logger = logging.getLogger(__name__)


def _coerce_uuid(value: Union[str, uuid.UUID]) -> uuid.UUID:
    return uuid.UUID(str(value)) if isinstance(value, str) else value


def _get_db(db: Optional[Session]) -> Session:
    if db is None:
        raise RuntimeError("Audit function called without a db session.")
    return db


# ── Public API ───────────────────────────────────────────────────────────────

def start_task(
    instance_id: Union[str, uuid.UUID],
    task_type: str,
    stage: Optional[str] = None,
    metadata: Optional[Dict[str, Any]] = None,
    db: Optional[Session] = None,
) -> uuid.UUID:
    """
    Creates audit row (status='queued') and a 'task_queued' event.
    Returns the audit row's UUID, reused as Celery's task_id.
    """
    instance_uuid = _coerce_uuid(instance_id)

    def _inner(session: Session) -> uuid.UUID:
        task = LabInstanceTask(
            lab_instance_id=instance_uuid,
            task_type=task_type,
            stage=stage,
            status="queued",
            enqueued_at=datetime.now(timezone.utc),
        )
        session.add(task)
        session.flush()

        event = LabInstanceEventLog(
            task_id=task.id,
            lab_instance_id=instance_uuid,
            event_type="task_queued",
            event_code="TASK_QUEUED",
            source=EventSource.SYSTEM.value,
            severity=EventSeverity.INFO.value,
            message=f"Task {task_type} queued for instance {instance_uuid}",
            created_at=datetime.now(timezone.utc),
            metadata_=metadata or {},
        )
        session.add(event)
        return task.id

    if db is not None:
        return _inner(db)

    with background_session() as fresh_db:
        return _inner(fresh_db)


def mark_running(
    task_id: Union[str, uuid.UUID],
    worker_pid: Optional[int] = None,
    worker_host: Optional[str] = None,
    db: Optional[Session] = None,
) -> None:
    task_uuid = _coerce_uuid(task_id)
    host = worker_host or socket.gethostname()

    def _inner(session: Session) -> None:
        task = (
            session.query(LabInstanceTask)
            .filter(LabInstanceTask.id == task_uuid)
            .first()
        )
        if not task:
            logger.warning("mark_running: Task %s not found", task_uuid)
            return

        task.status = "running"
        task.started_at = datetime.now(timezone.utc)
        task.worker_pid = worker_pid
        task.worker_host = host

        event = LabInstanceEventLog(
            task_id=task_uuid,
            lab_instance_id=task.lab_instance_id,
            event_type="task_started",
            event_code="TASK_STARTED",
            source=EventSource.WORKER.value,
            severity=EventSeverity.INFO.value,
            message=f"Task {task.task_type} started on worker {host}:{worker_pid}",
            created_at=datetime.now(timezone.utc),
            metadata_={},
        )
        session.add(event)

    if db is not None:
        _inner(db)
        return

    with background_session() as fresh_db:
        _inner(fresh_db)


def record_event(
    task_id: Union[str, uuid.UUID],
    instance_id: Union[str, uuid.UUID],
    event_type: str,
    message: str,
    event_code: Optional[str] = None,
    source: Optional[str] = None,
    severity: Optional[str] = None,
    metadata: Optional[Dict[str, Any]] = None,
    db: Optional[Session] = None,
) -> None:
    """
    Record an arbitrary event log entry with full severity/source support.
    """
    task_uuid = _coerce_uuid(task_id)
    instance_uuid = _coerce_uuid(instance_id)

    def _inner(session: Session) -> None:
        event = LabInstanceEventLog(
            task_id=task_uuid,
            lab_instance_id=instance_uuid,
            event_type=event_type,
            event_code=event_code or event_type.upper().replace(".", "_"),
            source=source or EventSource.WORKER.value,
            severity=severity or EventSeverity.INFO.value,
            message=message,
            created_at=datetime.now(timezone.utc),
            metadata_=metadata or {},
        )
        session.add(event)

    if db is not None:
        _inner(db)
        return

    with background_session() as fresh_db:
        _inner(fresh_db)


def update_task_progress(
    task_id: Union[str, uuid.UUID],
    progress_percent: int,
    db: Optional[Session] = None,
) -> None:
    """
    Update task progress (0-100). Useful for long-running stages like clone.
    """
    task_uuid = _coerce_uuid(task_id)

    def _inner(session: Session) -> None:
        task = (
            session.query(LabInstanceTask)
            .filter(LabInstanceTask.id == task_uuid)
            .first()
        )
        if task:
            task.progress_percent = max(0, min(100, progress_percent))

    if db is not None:
        _inner(db)
        return

    with background_session() as fresh_db:
        _inner(fresh_db)


def finish_task(
    task_id: Union[str, uuid.UUID],
    status: str,
    error_message: Optional[str] = None,
    db: Optional[Session] = None,
) -> None:
    valid_statuses = {"completed", "failed"}
    if status not in valid_statuses:
        raise ValueError(f"Invalid task status '{status}'")

    task_uuid = _coerce_uuid(task_id)

    def _inner(session: Session) -> None:
        task = (
            session.query(LabInstanceTask)
            .filter(LabInstanceTask.id == task_uuid)
            .first()
        )
        if not task:
            logger.warning("finish_task: Task %s not found", task_uuid)
            return

        task.status = status
        task.finished_at = datetime.now(timezone.utc)
        if error_message:
            task.error_message = error_message

        event_type = "task_completed" if status == "completed" else "task_failed"
        event_code = "TASK_COMPLETED" if status == "completed" else "TASK_FAILED"
        severity = EventSeverity.INFO.value if status == "completed" else EventSeverity.ERROR.value

        event = LabInstanceEventLog(
            task_id=task_uuid,
            lab_instance_id=task.lab_instance_id,
            event_type=event_type,
            event_code=event_code,
            source=EventSource.WORKER.value,
            severity=severity,
            message=error_message or f"Task {task.task_type} {status}",
            created_at=datetime.now(timezone.utc),
            metadata_={},
        )
        session.add(event)

    if db is not None:
        _inner(db)
        return

    with background_session() as fresh_db:
        _inner(fresh_db)